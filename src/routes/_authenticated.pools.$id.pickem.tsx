...
export const Route = createFileRoute("/_authenticated/pools/$id/pickem" as any)({
...
function PickemComponent() {
  const { id } = useParams({ from: "/_authenticated/pools/$id" } as any);
  ...
  const mutation = useMutation({
    mutationFn: upsertPickemPrediction,
    onSuccess: () => {
  ...
  return (
    <div className="container mx-auto p-4 space-y-6 pb-24">
      <header className="mb-6">
        ...
        <p className="text-muted-foreground">Escolha quem vence cada jogo. Acertos valem 3 pontos.</p>
      </header>
              ...
              <CardContent className="p-4 space-y-4">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <Button
                    ...
                    className={`h-auto flex-col py-3 gap-2 ${pred?.winner === 'home' ? 'ring-2 ring-primary ring-offset-2' : ''}`}
                  ...
                  >
                    ...
                    <span className="text-[10px] font-bold truncate w-full">{match.home_team.name}</span>
                  </Button>

                  <Button
                    variant={pred?.winner === 'draw' ? 'default' : 'outline'}
                    className={`h-auto flex-col py-3 gap-2 ${pred?.winner === 'draw' ? 'ring-2 ring-primary ring-offset-2' : ''}`}
                    disabled={isLocked || mutation.isPending}
                    onClick={() => handlePick(match.id, 'draw')}
                  >
                    <div className="h-6 flex items-center justify-center font-black">X</div>
                    <span className="text-[10px] font-bold uppercase">Empate</span>
                  </Button>

                  <Button
                    variant={pred?.winner === 'away' ? 'default' : 'outline'}
                    className={`h-auto flex-col py-3 gap-2 ${pred?.winner === 'away' ? 'ring-2 ring-primary ring-offset-2' : ''}`}
                    disabled={isLocked || mutation.isPending}
                    onClick={() => handlePick(match.id, 'away')}
                  >
                    <img src={match.away_team.flag_url} className="h-6 w-9 object-cover rounded shadow-sm" alt="" />
                    <span className="text-[10px] font-bold truncate w-full">{match.away_team.name}</span>
                  </Button>
                </div>
              </CardContent>
            </Card>
